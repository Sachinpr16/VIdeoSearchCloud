    ...
    def __init__(self, clip_type, use_temp=True, cache_dir='./cache_dir'):
        ...
        for k, v in clip_type.items():
            if k != 'language':
                # print(k, v)
                pretrained_ckpt = f'{v}'
                model = model_dict[k].from_pretrained(pretrained_ckpt, cache_dir=cache_dir, local_files_only=True)
                ...
                pretrained_ckpt = f'Image'
                model = model_dict['image'].from_pretrained(pretrained_ckpt, cache_dir=cache_dir, local_files_only=True)
                self.modality_encoder['language'] = model.text_model
                self.modality_proj['language'] = model.text_projection
